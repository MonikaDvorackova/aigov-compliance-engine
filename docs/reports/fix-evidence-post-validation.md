...
The workflow must print the full `POST /evidence` response body and HTTP status before failing, so evidence validation errors can be diagnosed precisely.

## Human approval gate

This change is approved because it improves CI observability only. It does not change compliance verdict logic, evidence requirements, tenant isolation, or backend authorization.
