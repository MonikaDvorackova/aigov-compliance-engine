...
This quickstart uses `GOVAI_API_KEYS` for simple local/dev setups. **Hosted / staging / production deployments should use `GOVAI_API_KEYS_JSON`** so each API key is mapped to a tenant/project and tenant isolation is enforced correctly.
