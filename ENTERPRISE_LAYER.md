...
- **Purpose:** **operational queue / org override only** — not a second compliance projection. Authoritative promotion readiness is **`GET /compliance-summary`** (`current_state` from the ledger). Successful **`/api/compliance-workflow*`** responses include **`decision_authority`** stating that the workflow row is subordinate to the ledger pipeline. The workflow does **not** append to **`audit_log.jsonl`**, change **`policy.rs`**, or substitute for **`POST /evidence`**. Emitting evidence events remains a **separate** step (e.g. Python/Makefile or any client).
