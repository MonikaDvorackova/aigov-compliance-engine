...
- **Purpose:** **operational queue / org override only** — not a second compliance projection. Authoritative promotion readiness is **`GET /compliance-summary`** (`current_state` from the ledger). List, register, get-one, and review responses include **`decision_authority`**. **`POST /api/compliance-workflow/:run_id/promotion`** returns **`ok`** + **`workflow`** only (no `decision_authority` field). The workflow does **not** append to **`audit_log.jsonl`**, change **`policy.rs`**, or substitute for **`POST /evidence`**. Emitting evidence events remains a **separate** step (e.g. Python/Makefile or any client).
